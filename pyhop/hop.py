...
from pyhop.helpers import print_goal, print_methods, print_operators
from pyhop.helpers import print_state
...
def search_operators(state, tasks, operators, methods, plan, task, depth,
                     verbose):
    if verbose > 2:
    ...
    if newstate:
        solution = seek_plan(newstate, tasks[1:], operators, methods,
                             plan + [task], depth + 1, verbose)
        if solution is not False:
            return solution


def search_methods(state, tasks, operators, methods, plan, task, depth,
                   verbose):
    if verbose > 2:
    ...
    for method in relevant:
        ...
        if subtasks is not False:
            solution = seek_plan(state, subtasks+tasks[1:], operators, methods,
                                 plan, depth+1, verbose)
            if solution is not False:
                return solution
...
def seek_plan(state, tasks, operators, methods, plan, depth, verbose=0):
    ...
    if task[0] in operators:
        return search_operators(state, tasks, operators, methods, plan, task,
                                depth, verbose)
    if task[0] in methods:
        return search_methods(state, tasks, operators, methods, plan, task,
                              depth, verbose)
    if verbose > 2:
